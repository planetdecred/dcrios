...
class PasswordSetupViewController: UIViewController, SeedCheckupProtocol, UITextFieldDelegate, StartUpPasswordProtocol,PinEnteredProtocol{
   ...
   //outlets
    ...
    @IBOutlet weak var passwordMatchIndicatorLabel: UILabel!
    ...
    // enccrypt wallet action when setting up wallet
    func onEncrypt() {
    ...
    func SetstartupPin_pas(){
        ...
        let finalkeypassData = (self.passwordTextField!.text! as NSString).data(using: String.Encoding.utf8.rawValue)!
        DispatchQueue.global(qos: .userInitiated).async { [weak self] in
            guard let this = self else { return }
            do {
    ...
    @IBAction func createPassBtn(_ sender: Any) {
        self.ActionButton()
    }
    ...
    @objc func textFieldDidChange(_: NSObject){
        if self.passwordTextField.text == self.confirmPasswordTextField.text {
            self.passwordMatchIndicatorLabel.textColor = #colorLiteral(red: 0.2537069321, green: 0.8615272641, blue: 0.7028611302, alpha: 1)
            self.passwordMatchIndicatorLabel.text = "PASSWORDS MATCH"
            self.okBtn.isEnabled = true
            self.okBtn.backgroundColor = UIColor(hex: "#007AFF")
            self.okBtn.setTitleColor(UIColor.white, for: .normal)
        }else{
            self.passwordMatchIndicatorLabel.textColor = #colorLiteral(red: 0.9254902005, green: 0.2352941185, blue: 0.1019607857, alpha: 1)
            self.passwordMatchIndicatorLabel.text = "PASSWORDS DO NOT MATCH"
    ...
    func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
        if (textField.tag == 5) { //password
            passwordStrength.progress = passwordStrengthMeasurer.strength(forPassword: textField.text) as! Float
            passwordStrength.progressTintColor = passwordStrengthMeasurer.strengthColor(forPassword: textField.text)
            passwordStrength.isHidden = false
            passwordStrengthLabel.isHidden = false
        } else {
            passwordMatchIndicatorLabel.isHidden = false
        }
        return true
    }
    ...
    // Set Text Header according to it current function
    func setHeader(){
        if (senders == "launcher") {
    ...
    // Detect screen size and set appropriate font size
    func setScreenFont(){
    ...
    func setFontSize(confirmPassLabeTxt: CGFloat, passwordLabelTxt: CGFloat, lbMatchIndicatorTxt: CGFloat,lbPasswordStrengthLabelTxt: CGFloat, okBtnTxt: CGFloat,headerTxt: CGFloat,tfPasswordTxt: CGFloat, tfConfirmPasswordTxt : CGFloat){
        self.confirmPasswordLabel.font = confirmPasswordLabel.font?.withSize(confirmPassLabeTxt)
        self.passwordLabel.font = passwordLabel.font?.withSize(passwordLabelTxt)
        self.passwordMatchIndicatorLabel.font = passwordMatchIndicatorLabel.font?.withSize(lbMatchIndicatorTxt)
        self.passwordStrengthLabel.font = passwordStrengthLabel.font.withSize(lbPasswordStrengthLabelTxt)
        self.headerText.font = headerText.font.withSize(headerTxt)
        self.okBtn.titleLabel?.font = .systemFont(ofSize: okBtnTxt)
        self.passwordTextField.font = .systemFont(ofSize: tfPasswordTxt)
        self.confirmPasswordTextField.font = .systemFont(ofSize: tfConfirmPasswordTxt)
    }
}
