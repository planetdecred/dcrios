...
class OverviewViewController: UIViewController{
    ...
    @IBOutlet weak var currentBalanceLabel: UILabel! {
        didSet { self.currentBalanceLabel.text = LocalizedStrings.currentTotalBalance }
    }
    ...
    @IBOutlet weak var recentActivityLabel: UILabel! {
        didSet { self.recentActivityLabel.text = LocalizedStrings.recentActivity}
    }
    ...
    @IBOutlet weak var walletStatusLabel: UILabel! {
        didSet { self.walletStatusLabel.text = LocalizedStrings.walletStatus }
    }
    ...
    @IBOutlet weak var syncStatusLabel: UILabel! {
        didSet { self.syncStatusLabel.text = (AppDelegate.walletLoader.isSynced) ? LocalizedStrings.walletSynced : LocalizedStrings.walletNotSynced }
    }
    ...
    override func viewDidLoad() {
        ...
        backupWarning.addGestureRecognizer(UITapGestureRecognizer(target: self, action: #selector(self.handleBackupWallet)))
        showSyncDetailsButton.addTarget(self, action: #selector(self.handleShowSyncToggle), for: .touchUpInside)
    ...
    private func setupInterface() {
        ...
        self.syncStatusImage.image = (AppDelegate.walletLoader.isSynced) ? UIImage(named: "icon-ok") : UIImage(named: "icon-cancel")
        ...
        syncStatusImage.image = (AppDelegate.walletLoader.isSynced) ? UIImage(named: "icon-ok") : UIImage(named: "icon-cancel")
    ...
    func updateCurrentBalance() {
        DispatchQueue.main.async {
            ...
            self.currentBalanceLabel.attributedText = Utils.getAttributedString(str: "\(totalAmountRoundedOff)", siz: 17.0, TexthexColor: GlobalConstants.Colors.TextAmount)
        }
    }
    
    func showNoTransactions() {
        let label = UILabel(frame: CGRect(x: 0, y: 0, width: self.recentTransactionsTableView.bounds.size.width, height: 56))
        label.text = LocalizedStrings.noTransactions
        label.textColor = UIColor(displayP3Red: 0.54, green: 0.59, blue: 0.65, alpha: 1)
        label.font = UIFont(name: "Source Sans Pro", size: 16)
        self.recentTransactionsTableView.isHidden = true
        self.recentActivitySection.addArrangedSubview(label)
        label.translatesAutoresizingMaskIntoConstraints = false
        label.clipsToBounds = true
        label.heightAnchor.constraint(equalToConstant: 56).isActive = true
        label.leadingAnchor.constraint(equalTo: recentActivitySection.leadingAnchor, constant: 24).isActive = true
    }
    ...
    func syncDetailsComponent() -> (view: UIView, constraints: [NSLayoutConstraint]) {
        ...
        detailsView.addSubview(connectedPeersText) // Connected peers count label
...
extension OverviewViewController: SyncProgressListenerProtocol {
    func onStarted(_ wasRestarted: Bool) {
        if (AppDelegate.walletLoader.wallet?.isSyncing() == true) {
            DispatchQueue.main.async {
                self.syncStatusImage.image = UIImage(named: "icon-syncing")
    ...
    func onSyncCompleted() {
        self.syncing = false
        self.syncStatusImage.image = UIImage(named: "icon-ok")
    ...
    func onSyncCanceled(_ willRestart: Bool) {
        self.syncing = false
        self.syncStatusImage.image = UIImage(named: "icon-cancel")
